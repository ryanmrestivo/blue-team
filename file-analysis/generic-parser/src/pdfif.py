...
def ProcessFile(filename, options, plugins):
    xmlDoc = PDFiD(filename, options.get("all",""), options.get("extra",""), options.get("disarm",""), options.get("force",""))
    return PDFiD2JSON(xmlDoc, options.get("force",False))
    if plugins == [] and options.get("select","") == '':
        Print(PDFiD2String(xmlDoc, options.get("force",""), options))
        return

    oPDFiD = cPDFiD(xmlDoc, options.get("force",""))
    if options.get("select",""):
        if options.get("force","") or not oPDFiD.errorOccured and oPDFiD.isPDF:
            pdf = oPDFiD
            try:
                selected = eval(options.get("select",""))
...
def PDFiDMain(filename):
    ...
    plugins = []
    LoadPlugins(options.get("plugins" , ""), options.get("verbose",""))
    ...
    #if options.get("scan",""):
    #    Scan(filename, options, options.get("plugins",""))
    #else:
    ProcessFile(filename, options, options.get("plugins",""))
