...
{ 
    ForEach ($line in (Get-Content -ReadCount 0 -Path $before.path))
    {
        ...
        if ($beforehashtable.ContainsKey( $path ))
        { Write-Verbose -Message ('Duplicate path in ReferenceFile: ' +  $path ) }
        else
        { $beforehashtable.Add( $path, $hash ) }
    }
    ...
    ForEach ($line in (Get-Content -ReadCount 0 -Path $after.path))
    {
        ...
        if ($afterhashtable.ContainsKey( $path ))
        { Write-Verbose -Message ('Duplicate path in DifferenceFile: ' +  $path ) }
        else
        { $afterhashtable.Add( $path, $hash ) }
    }
...
else #Assume a non-CSV file was created with one of the *DEEP tools (or anything with the same output format as MD5DEEP)
{
    ...
    ForEach ($line in (Get-Content -ReadCount 0 -Path $before.path))
    {
        ...
        if ($beforehashtable.ContainsKey( $arr[2] ))
        { Write-Verbose -Message ('Duplicate path in ReferenceFile: ' +  $arr[2] ) }
        else
        { $beforehashtable.Add( $arr[2], $arr[1] ) }
    }
    ...
    ForEach ($line in (Get-Content -ReadCount 0 -Path $after.path))
    {
        ...
        if ($afterhashtable.ContainsKey( $arr[2] ))
        { Write-Verbose -Message ('Duplicate path in DifferenceFile: ' +  $arr[2] ) }
        else
        { $afterhashtable.Add( $arr[2], $arr[1] ) }
    }
...
if ($SummaryOnly)
{
    $report = '' | select StartTime,FinishTime,RunTimeInSeconds,TotalDifferences,New,Missing,Changed,PerSecond
    $report.StartTime = $Start
    $report.FinishTime = Get-Date
    $report.RunTimeInSeconds = [MATH]::Round( (($report.FinishTime) - $Start).TotalSeconds, 3 )
    $report.TotalDifferences = $n + $m + $c 
    $report.New = $n 
    $report.Missing = $m 
    $report.Changed = $c 
    $report.PerSecond = [Math]::Round( ([Math]::Max($beforehashtable.count,$afterhashtable.count) / $report.RunTimeInSeconds), 1)
    $report 
}
