...
'              if it's not on the list, then distribute the script with Group Policy And
'              make it a shutdown or startup script.
'       Notes: There are a few root certs that should not be removed because they are necessary
'              for Windows to function (KB293781).  These necessary certs have been removed
'              from the list and pasted at the bottom of the script.
...
'Here are the necessary root CA certs as described in KB293781:
...
'sCertKeys = sCertKeys & "24A40A1F573643A67F0A4B0749F6A22BF28ABB6B:"  'VeriSign Commercial Software Publishers CA (the one used to issue the fraudulent software publisher's certs (Q293817)) Caution: Very many legitimate publishers' certs are subordinate to this.
