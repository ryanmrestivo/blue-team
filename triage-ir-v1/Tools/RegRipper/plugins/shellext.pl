#-----------------------------------------------------------
# shellext
#   Plugin to get approved shell extensions list from the Software hive
#
...
#   then goes to the Classes\CLSID key to get the path to the associated DLL.
#   The intruder chose a shell extension that did not have an explicit path,
#   so when explorer.exe looked for it, it started in the C:\Windows dir,
#   and never got to the legit DLL in the C:\Windows\system32 dir.
#
# Change history
...
sub pluginmain {
    ...
    ::rptMsg("shellext v.".$VERSION); # 20110830 [fpi] + banner
